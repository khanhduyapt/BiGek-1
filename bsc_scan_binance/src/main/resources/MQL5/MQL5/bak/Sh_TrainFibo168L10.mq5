...
//string Trend_H4C0 = (cur_price>iOpen(symbol,PERIOD_H4,0)?TREND_BUY:TREND_SEL);
   ...
   ENUM_TIMEFRAMES PERIOD_MACD_FOLLOWING = PERIOD_H1;
   bool IS_MACD_FOLLOWING = GetGlobalVariable(BtnTrendFollow+symbol)==AUTO_TRADE_ON;
   ...
   string trend_macd_following = "";
   if(IS_MACD_FOLLOWING)
     {
      trend_macd_following = get_trend_by_histogram(symbol,PERIOD_MACD_FOLLOWING);

      if(trend_macd_following==TREND_SEL)
         IS_CYCLE_BUY=false;
      if(trend_macd_following==TREND_BUY)
         IS_CYCLE_SEL=false;
     }
...
//---------------------------------------------------------------------------------------------------------
   ...
   create_trend_line("NEXT_BUY",time,next_buy,time+TIME_OF_ONE_D1_CANDLE,next_buy,clrBlue,STYLE_SOLID,3);
   create_trend_line("NEXT_SEL",time,next_sel,time+TIME_OF_ONE_D1_CANDLE,next_sel,clrRed, STYLE_SOLID,3);
...
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      if(IS_CYCLE_BUY && (global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask))
        {
         count_buy += 1;
            ...
            if(count_buy>=START_EXIT_L)
              {
               ...
               m_trade.BuyLimit(vol,MathMin(bid-0.25,iLow(symbol,PERIOD_D1,0)),symbol,0.0,TP_BUY,0,0,comment_buy);
              }
      ...
      if(profit_buy>risk_min)
        {
         if((count_buy>=START_EXIT_L) || (IS_CYCLE_BUY==false))
           {
      ...
      if(potential_profit_buy<0)
        {
         ...
         ModifyTp(symbol,TREND_BUY,TREND_BUY,last_tp_buy+0.25);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   ...
   if(count_sel>0)
     {
            ...
            if(count_sel>=START_EXIT_L)
              {
               ...
               m_trade.SellLimit(vol,MathMax(ask+0.25,iHigh(symbol,PERIOD_D1,0)),symbol,0.0,TP_SEL,0,0,comment_sel);
              }
      ...
      if(profit_sel>risk_min)
        {
         if((count_sel>=START_EXIT_L) || (IS_CYCLE_SEL==false))
           {
      ...
      if(potential_profit_sel<0)
        {
         ...
         ModifyTp(symbol,TREND_SEL,TREND_SEL,last_tp_sel-0.25);
        }
     }
//---------------------------------------------------------------------------------------------------------
   if(IS_ALLOW_HEDGING)
      ...
      if(vol_negertive_buy>0 && (vol_negertive_buy-vol_heg_buy)>0)
        {
         ...
         Open_Position(symbol,TREND_SEL,(vol_negertive_buy-vol_heg_buy),0.0,TP_SEL_L1,comment_buy);
      ...
      if(vol_negertive_sel>0 && (vol_negertive_sel-vol_heg_sel)>0)
        {
         ...
         Open_Position(symbol,TREND_BUY,(vol_negertive_sel-vol_heg_sel),0.0,TP_BUY_L1,comment_sel);
   ...
   if(true)
     {
      if(count_buy>0 && is_same_symbol(total_comment_buy,comment_L001_buy)==false)
        {
         ClosePositionByTrend(symbol,TREND_BUY);
         Alert("STOP_LOSS " + symbol + "  " + total_comment_buy);
         return;
        }
      ...
      if(count_sel>0 && is_same_symbol(total_comment_sel,comment_L001_sel)==false)
        {
         ClosePositionByTrend(symbol,TREND_SEL);
         Alert("STOP_LOSS " + symbol + "  " + total_comment_sel);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   ...
   if(is_cur_tab)
     {
      ...
      color clrColorTrendFollowing = IS_MACD_FOLLOWING?clrLightGreen:clrLightGray;
      string strLblTrendFollowing = "(Follow " + get_time_frame_name(PERIOD_MACD_FOLLOWING) + ") " + trend_macd_following + " " + (IS_MACD_FOLLOWING?"ON":"OFF");
      createButton(BtnTrendFollow, strLblTrendFollowing, x_ref_btn+100, (int)(y_ref_btn/2), 150, 20, clrBlack, clrColorTrendFollowing, 7);
      ...
      string stoc_main = "(" + format_double_to_string(AMP_TRADE,digits-1) + "/" + format_double_to_string(AMP_TP_L1,digits-1) + ")";
