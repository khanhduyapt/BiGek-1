...
double AMP_STOP = 24.38;
double AMP_RECOVER = 24.38;
double AMP_TP_L1 = 3.16;
double AMP_TRAILING_STOP = 1.683;   //=ATR_M5(200)
double FIBO = 2.618;
double INIT_VOLUME=0.01;
int START_EXIT_L = 2;
double SL_L001 = 100.0;
double FIXED_SL = 10000.00; // 100$
...
//DeleteAllObjects();
   ...
   if(false && is_same_symbol(symbol,"XAU"))
     {
      SetGlobalVariable(BtnAutoBuy+symbol, AUTO_TRADE_OFF);
...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   create_trend_line("OPEN_D0",time,open_d0,time+TIME_OF_ONE_D1_CANDLE,open_d0,clrBlack);
   create_trend_line("END_SEL",time,open_d0+AMP_STOP,time+TIME_OF_ONE_D1_CANDLE,open_d0+AMP_STOP,clrBlue);
   create_trend_line("END_BUY",time,open_d0-AMP_STOP,time+TIME_OF_ONE_D1_CANDLE,open_d0-AMP_STOP,clrRed);

   string Trend_M5C0 = (iClose(symbol,PERIOD_M5,0)>iOpen(symbol,PERIOD_M5,0)?TREND_BUY:TREND_SEL);
   string Trend_M5C1 = (Trend_M5C0==(iClose(symbol,PERIOD_M5,1)>iOpen(symbol,PERIOD_M5,1)?TREND_BUY:TREND_SEL))?Trend_M5C0:"";
   string Trend_H4C0 = (cur_price>iOpen(symbol,PERIOD_H4,0)?TREND_BUY:TREND_SEL);

   bool IS_CYCLE_BUY = (GetGlobalVariable(BtnAutoBuy+symbol)==AUTO_TRADE_ON) || IS_TESTING;
   bool IS_CYCLE_SEL = (GetGlobalVariable(BtnAutoSel+symbol)==AUTO_TRADE_ON) || IS_TESTING;

   double ATR_200D = get_ATR(symbol,PERIOD_H4);
   AMP_RECOVER = MathMax(AMP_TP_L1*3,ATR_200D);

   if(open_d0+AMP_STOP<bid)
      IS_CYCLE_BUY=false;
   if(open_d0-AMP_STOP>ask)
      IS_CYCLE_SEL=false;

   ObjectDelete(0,"NEWS");
   if(IS_TESTING==false && is_economic_news())
     {
      IS_CYCLE_BUY=false;
      IS_CYCLE_SEL=false;
      create_lable_simple("NEWS","Disable trading due to economic news",open_d0);
     }
//---------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------
   ...
   ObjectDelete(0,"MASK_RECOVER_BUY");
   ObjectDelete(0,"MASK_RECOVER_SEL");
   double global_min_recover_buy=0,global_max_recover_sel=0;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
            ...
            if(is_same_symbol(comment, TREND_BUY))
              {
               ...
               if(is_same_symbol(comment,MASK_RECOVER))
                 {
                  count_recover_buy+=1;
                  if(global_min_recover_buy==0 || global_min_recover_buy>m_position.PriceOpen())
                     global_min_recover_buy=m_position.PriceOpen();

                  create_label_simple("MASK_RECOVER_BUY", "--- "+comment,m_position.PriceOpen(),clrBlue,time+TIME_OF_ONE_D1_CANDLE);
                 }
               ...
               if(is_same_symbol(m_position.TypeDescription(),TREND_BUY))
                 {
                  ...
                  if(m_position.TakeProfit() > 0 && m_position.TakeProfit() < m_position.PriceOpen())
                     vol_negertive_buy += m_position.Volume();

                  double new_stop_level = bid - AMP_TRAILING_STOP;
                  if((temp_profit>risk_1L) && (m_position.PriceOpen() < new_stop_level) && (m_position.StopLoss()==0 || m_position.StopLoss() < new_stop_level))
                     m_trade.PositionModify(m_position.Ticket(),new_stop_level,m_position.TakeProfit());
                  ...
                  potential_profit_buy += calcPotentialTradeProfit(symbol, TREND_BUY, m_position.PriceOpen(), m_position.TakeProfit(), m_position.Volume());

                  if(global_min_entry_buy == 0 || global_min_entry_buy > m_position.PriceOpen())
                    {
                     last_tp_buy = m_position.TakeProfit();
            ...
            if(is_same_symbol(comment, TREND_SEL))
              {
               ...
               if(is_same_symbol(comment,MASK_RECOVER))
                 {
                  count_recover_sel+=1;
                  if(global_max_recover_sel==0 || global_max_recover_sel<m_position.PriceOpen())
                     global_max_recover_sel=m_position.PriceOpen();

                  create_label_simple("MASK_RECOVER_SEL", "--- "+comment,m_position.PriceOpen(),clrBlue,time+TIME_OF_ONE_D1_CANDLE);
                 }
               ...
               if(is_same_symbol(m_position.TypeDescription(),TREND_SEL))
                 {
                  ...
                  potential_profit_sel += calcPotentialTradeProfit(symbol, TREND_SEL, m_position.PriceOpen(), m_position.TakeProfit(), m_position.Volume());
                  ...
                  if(global_max_entry_sel == 0 || global_max_entry_sel < m_position.PriceOpen())
                    {
                     last_tp_sel = m_position.TakeProfit();
     ...
     {
      if(count_recover_buy > (int)GetGlobalVariable(MAX_L_BUY))
         SetGlobalVariable(MAX_L_BUY, (double)count_recover_buy);

      if(count_recover_sel > (int)GetGlobalVariable(MAX_L_SEL))
         SetGlobalVariable(MAX_L_SEL, (double)count_recover_sel);
...
//---------------------------------------------------------------------------------------------------------
   bool ALLOW_HEDGING = false;
   if(ALLOW_HEDGING)
     {
      if((count_buy>=START_EXIT_L) || (count_sel>=START_EXIT_L) || (count_heg_buy>0) || (count_heg_sel>0))
        {
         if(global_profit>0)
           {
            ClosePositionByTrend(symbol,TREND_BUY);
            ClosePositionByTrend(symbol,TREND_SEL);
            Alert(get_vntime() + " Redo.");
            printf(get_vntime() + " Redo.");
            return;
           }
        }

      if(((count_buy>7) || (count_sel>7)))
        {
         double profit_today = CalculateTodayProfitLoss();
         if(profit_today+profit_buy+profit_sel>0)
           {
            ClosePositionByTrend(symbol,TREND_BUY);
            ClosePositionByTrend(symbol,TREND_SEL);
            Alert(get_vntime() + " Today is a waste.");
            printf(get_vntime() + " Today is a waste.");
            return;
           }
        }

      if(vol_negertive_buy>0 && (vol_negertive_buy-vol_heg_buy)>0)
        {
         count_buy+=1;
         string comment_buy = create_comment(MASK_HEGING, TREND_BUY, count_buy);

         Open_Position(symbol,TREND_SEL,(vol_negertive_buy-vol_heg_buy),0.0,0.0,comment_buy);

         return;
        }

      if(vol_negertive_sel>0 && (vol_negertive_sel-vol_heg_sel)>0)
        {
         count_sel+=1;
         string comment_sel = create_comment(MASK_HEGING, TREND_SEL, count_sel);

         Open_Position(symbol,TREND_BUY,(vol_negertive_sel-vol_heg_sel),0.0,0.0,comment_sel);
...
//---------------------------------------------------------------------------------------------------------
   ...
   double next_recover_buy=count_recover_buy==0?global_max_entry_buy-AMP_RECOVER:global_min_recover_buy-AMP_RECOVER;
   double next_recover_sel=count_recover_sel==0?global_min_entry_sel+AMP_RECOVER:global_max_recover_sel+AMP_RECOVER;

   create_trend_line("NEXT_RECOVER_BUY",time,next_recover_buy,time+TIME_OF_ONE_D1_CANDLE,next_recover_buy,clrBlue,STYLE_SOLID,3);
   create_trend_line("NEXT_RECOVER_SEL",time,next_recover_sel,time+TIME_OF_ONE_D1_CANDLE,next_recover_sel,clrRed, STYLE_SOLID,3);
   ...
   string lblReBuy = "Rec.B="+format_double_to_string(range_buy,digits-1)+" / "+(string)count_buy+"L " + MASK_RECOVER+(string)count_recover_buy + " Vol:" + format_double_to_string(vol_recover_buy,2);
   string lblReSel = "Rec.S="+format_double_to_string(range_sel,digits-1)+" / "+(string)count_sel+"L " + MASK_RECOVER+(string)count_recover_sel + " Vol:" + format_double_to_string(vol_recover_sel,2);

   create_lable_simple("RANGE_BUY",lblReBuy,next_recover_buy,clrBlack,time+TIME_OF_ONE_D1_CANDLE);
   create_lable_simple("RANGE_SEL",lblReSel,next_recover_sel,clrBlack,time+TIME_OF_ONE_D1_CANDLE);
//---------------------------------------------------------------------------------------------------------
   double TP_BUY = ask+AMP_TP_L1;
   double TP_SEL = bid-AMP_TP_L1;

   if(IS_CYCLE_BUY && ((count_buy==0) || (global_min_entry_buy-AMP_TP_L1>ask)))
     {
      count_buy+=1;
      string comment_buy = create_comment(MASK_NORMAL,TREND_BUY,count_buy);
      bool open_buy=Open_Position(symbol,TREND_BUY,INIT_VOLUME,0.0,TP_BUY,comment_buy);
      return;
     }

   if(IS_CYCLE_SEL && ((count_sel==0) || (global_max_entry_sel+AMP_TP_L1<bid)))
     {
      count_sel+=1;
      string comment_sel = create_comment(MASK_NORMAL,TREND_SEL,count_sel);
      bool open_sel=Open_Position(symbol,TREND_SEL,INIT_VOLUME,0.0,TP_SEL,comment_sel);
      return;
     }
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      if((next_recover_buy>ask)
         //&& (Trend_H4C0==TREND_BUY) && (Trend_M5C1==TREND_BUY)// && (count_sel>1)
        )
        {
         count_buy+=1;
         count_recover_buy+=1;

         string comment_buy = MASK_RECOVER+append1Zero(count_recover_buy)+create_comment(MASK_NORMAL,TREND_BUY,count_buy);

         Open_Position(symbol,TREND_BUY,vol_recover_buy,0.0,0.0,comment_buy);
         return;
        }
     }
   ...
   if(count_sel>0)
     {
      if((next_recover_sel<bid)
         //&& (Trend_H4C0==TREND_SEL) && (Trend_M5C1==TREND_SEL)// && (count_buy>1)
        )
        {
         count_sel += 1;
         count_recover_sel+=1;

         string comment_sel = MASK_RECOVER+append1Zero(count_recover_sel)+create_comment(MASK_NORMAL,TREND_SEL,count_sel);

         Open_Position(symbol,TREND_SEL,vol_recover_sel,0.0,0.0,comment_sel);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   if(profit_buy>risk_min)
      if((count_recover_buy>0) || (IS_CYCLE_BUY==false))
        {
   ...
   if(profit_sel>risk_min)
      if((count_recover_sel>0) || (IS_CYCLE_SEL==false))
        {
   ...
   if(is_cur_tab)
     {
      ...
      if(cur_price>0&&global_max_entry_buy>0)
         step_buy = NormalizeDouble(MathAbs(cur_price-global_max_entry_buy)/AMP_TP_L1, 2);

      double step_sel = 0.01;
      if(cur_price>0&&global_min_entry_sel>0)
         step_sel = NormalizeDouble(MathAbs(cur_price-global_min_entry_sel)/AMP_TP_L1, 2);
      ...
      string stoc_main = "(" + format_double_to_string(AMP_TP_L1,digits-1) + "/" + format_double_to_string(ATR_200D,digits-1) + ")";

      color clrNewCycleColorBuy = count_buy>0 ? clrLightGreen : clrLightGray;
      string lableBuy = tf + " L1 " + stoc_main + strCountB + " (Ne." + format_double_to_string(vol_negertive_buy,2) + ")" + " Est: "+ format_double_to_string(potential_profit_buy,2) + "$";;
      ...
      createButton(BtnDelLimitBuy, "Del.B", x_ref_btn+40, y_ref_btn-150, 50, 20, clrBlack, clrLightGray, 7);
      createButton(BtnSendLimitBuy, "Limit.5L.Buy." + format_double_to_string(AMP_TP_L1,digits-1), x_ref_btn+100, y_ref_btn-150, 150, 20, clrBlack, clrLightGray, 7);
      createButton(BtnSendLimitSel, "Limit.5L.Sel." + format_double_to_string(AMP_TP_L1,digits-1), x_ref_btn+100, 150, 150, 20, clrBlack, clrLightGray, 7);
      createButton(BtnDelLimitSel, "Del.S", x_ref_btn+40, 150, 50, 20, clrBlack, clrLightGray, 7);

      color clrNewCycleColorSel = count_sel>0 ? clrMistyRose : clrLightGray;
      string lableSel = tf + " L1 " + stoc_main + strCountS + " (Ne." + format_double_to_string(vol_negertive_sel,2) + ")" + " Est: "+ format_double_to_string(potential_profit_sel,2) + "$";
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Comment(), BOT_SHORT_NM) &&
           ...
           {
            bool is_same_tp = format_double_to_string(tp_price, digits-1) == format_double_to_string(m_position.TakeProfit(), digits-1) ;
...
void ClosePositionByTrend(string symbol, string TREND)
  {
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Comment(), BOT_SHORT_NM) &&
            is_same_symbol(m_position.Symbol(), symbol) && is_same_symbol(m_position.Comment(),TREND))
           {
            int demm = 1;
            while(demm<5)
              {
...
string GetComments()
  {
   ...
   str_comments += "    TP_L1: " + format_double_to_string(AMP_TP_L1, digits)
                   + "  AMP_RECOVER: "+format_double_to_string(AMP_RECOVER, digits)
                   + "  SL1: " + (string)SL_L001
