...
bool IS_ALLOW_TRAILING_STOP=true;
//AMP_TP_L1=6$: TP L12 66Giá, TK:60$
//AMP_TP_L1=7$: TP L12 77Giá, TK:68$
//AMP_TP_L1=8$: TP L12 88Giá, TK:76$
double FIBO = 1.618;
int START_EXIT_Eq = 10;
int START_EXIT_XL = 11;
double INIT_VOLUME=0.01;
double AMP_STOP = 45;
double AMP_TRADE = 10.0;
double AMP_SL_L1 = 30;
double AMP_TP_L1 = AMP_TRADE;
double AMP_TP_Lx = AMP_TRADE;
double AMP_TRAILING_STOP = 1.618;
double AMP_HEDGING = 10.0;
double FIXED_SL = 4000.00; // 40$
string BOT_SHORT_NM = "(D1A6)";
//-----------------------------------------------------------------------------
...
int y_ref_btn = (int) MathRound(ChartGetInteger(0, CHART_HEIGHT_IN_PIXELS))-20;
//+------------------------------------------------------------------+
   ...
   if(true)
     {
      ...
      iMA(symbol,PERIOD_H1,10,0,MODE_SMA,0);
     }
...
//double price_lowest_2d=MathMin(open_d0-AMP_STOP,MathMin(iLow(symbol,PERIOD_D1,0),iLow(symbol,PERIOD_D1,1)));
//double price_higest_2d=MathMax(open_d0+AMP_STOP,MathMax(iHigh(symbol,PERIOD_D1,0),iHigh(symbol,PERIOD_D1,1)));
   ...
   create_trend_line("OPEN_D0",time,open_d0,time+TIME_OF_ONE_D1_CANDLE,open_d0,clrBlack,STYLE_SOLID);

   double price_end_buy=MathAbs(open_d0+AMP_STOP);
   double price_end_sel=MathAbs(open_d0-AMP_STOP);
   create_trend_line("Sel2Buy",time,price_end_sel,time+TIME_OF_ONE_D1_CANDLE,price_end_sel,clrRed,STYLE_SOLID);
   create_trend_line("Buy2Sel",time,price_end_buy,time+TIME_OF_ONE_D1_CANDLE,price_end_buy,clrBlue,STYLE_SOLID);
   ...
   CandleData arrHeikenD1[];
   get_arr_heiken(symbol,PERIOD_D1,arrHeikenD1,50);

   string trend_d1 = arrHeikenD1[0].trend_by_ma10==arrHeikenD1[0].trend_heiken?arrHeikenD1[0].trend_by_ma10:"";

   CandleData arrHeikenH1[];
   get_arr_heiken(symbol,PERIOD_H1,arrHeikenH1,50);
   string trend_ma10h1 = arrHeikenH1[1].trend_by_ma10;
   string trend_c0h1 = (bid>iOpen(symbol,PERIOD_H1,0))?TREND_BUY:TREND_SEL;


   string trend_stoc_h27="",trend_stoc_h18="",trend_macd_h26="";
   int count_stoc_h27=0,value_stoc_h27=0, count_stoc_h18=0,value_stoc_h18=0, count_macd_h26=0,value_macd_h26=0;
   get_trend_by_stoc3c(symbol,PERIOD_H1,trend_stoc_h27,count_stoc_h27,value_stoc_h27,27,12,9,1);
   get_trend_by_stoc3c(symbol,PERIOD_H1,trend_stoc_h18,count_stoc_h18,value_stoc_h18,18,12,6,1);
//get_trend_by_macd3c(symbol,PERIOD_H1,trend_macd_h26,count_macd_h26,value_macd_h26,12,26,9);
//+"  Macd "+getShortName(trend_macd_h26)+",c"+(string)count_macd_h26+", v"+(string)value_macd_h26

   string str_trend_h1="(D1) Ma "+getShortName(arrHeikenD1[0].trend_by_ma10) +",c"+(string)arrHeikenD1[0].count_ma10
                       + " (H1) Ma "+getShortName(trend_ma10h1) +",c"+(string)arrHeikenH1[1].count_ma10
                       +"  (Sto27) "+getShortName(trend_stoc_h27)+",c"+(string)count_stoc_h27+",v"+(string)value_stoc_h27
                       +"  (Sto18) "+getShortName(trend_stoc_h18)+",c"+(string)count_stoc_h18+",v"+(string)value_stoc_h18
                       ;

   color clrColor=is_same_symbol(str_trend_h1,TREND_BUY)?clrBlue:clrRed;
   createButton("str_trend_h1",str_trend_h1,x_ref_btn/2,80,450,20,clrColor,clrWhite);

   if(is_same_symbol(trend_ma10h1,TREND_SEL) &&
      is_same_symbol(arrHeikenH1[0].trend_by_ma10,TREND_SEL) &&
      is_same_symbol(arrHeikenH1[0].trend_heiken,TREND_SEL))
      IS_CYCLE_BUY=false;
   if(is_same_symbol(trend_ma10h1,TREND_BUY) &&
      is_same_symbol(arrHeikenH1[0].trend_by_ma10,TREND_BUY) &&
      is_same_symbol(arrHeikenH1[0].trend_heiken,TREND_BUY))
      IS_CYCLE_SEL=false;

   bool allow_sel2buy=false;
   bool allow_buy2sel=false;
   if((price_end_buy<bid))
     {
      IS_CYCLE_BUY=false;

      //if(arrHeikenH1[0].trend_heiken==TREND_SEL)
        {
         allow_buy2sel=true;
         IS_CYCLE_SEL=true;
        }
   ...
   if((price_end_sel>ask))
     {
      ...
      //if(arrHeikenH1[0].trend_heiken==TREND_BUY)
        {
         allow_sel2buy=true;
         IS_CYCLE_BUY=true;
        }
...
//---------------------------------------------------------------------------------------------------------
   ...
   bool is_moved_2_entry_buy=true,is_moved_2_entry_sel=true;
   int count_buy = 0, count_sel = 0, count_recover_buy = 0, count_recover_sel = 0, count_heg_buy = 0, count_heg_sel = 0;
   double vol_negertive_buy = 0, vol_negertive_sel = 0,profit_negertive_buy = 0, profit_negertive_sel = 0;
   string str_vol_buy = "", str_vol_sel = "";
   string last_comment_buy = "", last_comment_sel = "";
   string total_comment_buy = "", total_comment_sel = "";
   string total_comment_hedging_buy = "", total_comment_hedging_sel = "";
   double last_tp_buy = 0, last_tp_sel = 0;
   double vol_buy = 0, vol_sel = 0, profit_buy = 0, profit_sel = 0;
   double vol_negertive_heg_buy = 0, vol_negertive_heg_sel = 0, profit_negertive_heg_buy = 0, profit_negertive_heg_sel = 0;
            ...
            if(is_same_symbol(comment, TREND_BUY))
              {
               ...
               if(is_same_symbol(comment,MASK_HEGING))
                 {
                  count_heg_buy += 1;
                  total_comment_hedging_buy+=comment;

                  if(temp_profit<0)
                    {
                     vol_negertive_heg_buy += m_position.Volume();
                     profit_negertive_heg_buy+=temp_profit;
                    }

                  if(m_position.TakeProfit()!= m_position.PriceOpen())
                     is_moved_2_entry_buy = false;

                  create_lable_simple((string)m_position.Ticket(),"<"+m_position.Comment(),m_position.PriceOpen(),clrBlue,m_position.Time());
                 }
               else
                  if(is_same_symbol(m_position.TypeDescription(),TREND_BUY))
                    {
                     ...
                     if(temp_profit < 0)
                       {
                        ...
                        profit_negertive_buy+=temp_profit;
                       }
                     //--------------------------------------MOVE_TP_BUY----------------------------------------
                     ...
                     //--------------------------------------TRAILING_STOP--------------------------------------
                     if(IS_ALLOW_TRAILING_STOP)
                        if((temp_profit>risk_1L) && (m_position.Volume()>=INIT_VOLUME))
                          {
                           double new_stop_level = bid - AMP_TRAILING_STOP;
                           if((m_position.PriceOpen() < new_stop_level))
            ...
            if(is_same_symbol(comment, TREND_SEL))
              {
               ...
               if(is_same_symbol(comment,MASK_HEGING))
                 {
                  count_heg_sel += 1;
                  total_comment_hedging_sel+=comment;

                  if(temp_profit<0)
                    {
                     vol_negertive_heg_sel += m_position.Volume();
                     profit_negertive_heg_sel+=temp_profit;
                    }

                  if(m_position.TakeProfit()!= m_position.PriceOpen())
                     is_moved_2_entry_sel = false;

                  create_lable_simple((string)m_position.Ticket(),"<"+m_position.Comment(),m_position.PriceOpen(),clrRed,m_position.Time());
                 }
               else
                     ...
                     if(temp_profit < 0)
                       {
                        ...
                        profit_negertive_sel+=temp_profit;
                       }
                     //--------------------------------------MOVE_TP_SEL----------------------------------------
                     //Nếu giá gần đạt TP hiện tại (cách khoảng 2$), dịch TP lên thêm 3$
                     //double new_tp_level = current_tp - AMP_MOVE_TP;
...
//---------------------------------------------------------------------------------------------------------
   ...
   create_label_simple("Sel2Buy_","Sel2Buy",price_end_sel,clrBlack,time);
   create_label_simple("Buy2Sel_","Buy2Sel",price_end_buy,clrBlack,time);

   create_lable_simple("RANGE_BUY",lblReBuy,next_buy,clrBlack,time-TIME_OF_ONE_D1_CANDLE*2);
   create_lable_simple("RANGE_SEL",lblReSel,next_sel,clrBlack,time-TIME_OF_ONE_D1_CANDLE*2);

   create_lable_simple("TP_BUY","TP_BUY: "+(string)last_tp_buy,last_tp_buy,clrBlack,time-TIME_OF_ONE_D1_CANDLE*3);
   create_lable_simple("TP_SEL","TP_SEL: "+(string)last_tp_sel,last_tp_sel,clrBlack,time-TIME_OF_ONE_D1_CANDLE*3);
//---------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------
   if((count_heg_buy+count_heg_sel>0) && (global_profit>risk_min))
     {
      ClosePositionByTrend(symbol,TREND_BUY);
      ClosePositionByTrend(symbol,TREND_SEL);
      Alert("DE_HEDGING   " + symbol + "    Profit:"+ (string)(int)(global_profit) + "$");
      return;
     }
//---------------------------------------------------------------------------------------------------------
   if(false)
     {
      if(allow_sel2buy && (count_buy==0))
        {
         count_buy = 1;
         double SL_L1 = bid-AMP_SL_L1;
         string comment_buy = create_comment("S2B"+MASK_NORMAL,TREND_BUY,count_buy);

         Open_Position(symbol,TREND_BUY,INIT_VOLUME,SL_L1,TP_BUY_L1,comment_buy);
         return;
        }

      if(allow_buy2sel  && (count_sel==0))
        {
         count_sel = 1;
         double SL_L1 = ask+AMP_SL_L1;
         string comment_sel = create_comment("B2S"+MASK_NORMAL,TREND_SEL,count_sel);

         Open_Position(symbol,TREND_SEL,INIT_VOLUME,SL_L1,TP_SEL_L1,comment_sel);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   if(true)
     {
      int ENTRY_CANDLE=5;
      if((trend_d1==TREND_BUY) && IS_CYCLE_BUY && (count_buy==0) && (trend_c0h1==TREND_BUY))
         if((is_same_symbol(trend_stoc_h27,TREND_BUY) && count_stoc_h27<=ENTRY_CANDLE && value_stoc_h27<70 && is_same_symbol(trend_stoc_h27,trend_ma10h1)) ||
            (is_same_symbol(trend_stoc_h18,TREND_BUY) && count_stoc_h18<=ENTRY_CANDLE && value_stoc_h18<70 && is_same_symbol(trend_stoc_h18,trend_ma10h1)) ||
            (is_same_symbol(trend_macd_h26,TREND_BUY) && count_macd_h26<=ENTRY_CANDLE && is_same_symbol(trend_macd_h26,trend_ma10h1))
           )
           {
            count_buy = 1;
            double SL_L1 = bid-AMP_SL_L1;
      ...
      if((trend_d1==TREND_SEL) && IS_CYCLE_SEL && (count_sel==0) && (trend_c0h1==TREND_SEL))
         if((is_same_symbol(trend_stoc_h27,TREND_SEL) && count_stoc_h27<=ENTRY_CANDLE && value_stoc_h27>30 && is_same_symbol(trend_stoc_h27,trend_ma10h1)) ||
            (is_same_symbol(trend_stoc_h18,TREND_SEL) && count_stoc_h18<=ENTRY_CANDLE && value_stoc_h18>30 && is_same_symbol(trend_stoc_h18,trend_ma10h1)) ||
            (is_same_symbol(trend_macd_h26,TREND_SEL) && count_macd_h26<=ENTRY_CANDLE && is_same_symbol(trend_macd_h26,trend_ma10h1))
           )
           {
            count_sel = 1;
            double SL_L1 = ask+AMP_SL_L1;
...
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      if((trend_c0h1==TREND_BUY) && (global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask) && CanOpenDCA(symbol,PERIOD_H1,last_time_buy))
        {
         count_buy += 1;
         ...
         bool result = Open_Position(symbol,TREND_BUY,vol,0.0,0.0,comment_buy);
      ...
      if(profit_buy>risk_1L)
        {
         if((count_buy>=MathMin(START_EXIT_Eq,START_EXIT_XL)) || (IS_CYCLE_BUY==false))
           {
      ...
      if(potential_profit_buy<risk_1L*count_buy)
        {
         if(last_tp_buy==0)
            last_tp_buy=global_max_entry_buy+AMP_TP_L1;

         ModifyTp(symbol,TREND_BUY,TREND_BUY,last_tp_buy+AMP_TP_L1);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------
   if(count_sel>0)
     {
      if((trend_c0h1==TREND_SEL) && (global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid) && CanOpenDCA(symbol,PERIOD_H1,last_time_sel))
        {
         count_sel += 1;
         ...
         bool result=Open_Position(symbol,TREND_SEL,vol,0.0,0.0,comment_sel);
      ...
      if(profit_sel>risk_1L)
        {
         if((count_sel>=MathMin(START_EXIT_Eq,START_EXIT_XL)) || (IS_CYCLE_SEL==false))
           {
      ...
      if(potential_profit_sel<risk_1L*count_sel)
        {
         if(last_tp_sel==0)
            last_tp_sel=global_min_entry_sel-AMP_TP_L1;

         ModifyTp(symbol,TREND_SEL,TREND_SEL,last_tp_sel-AMP_TP_L1);
        }
     }
//---------------------------------------------------------------------------------------------------------
     {
   ...
   if(is_cur_tab)
     {
      ...
      //color clrColorTrendFollowing = IS_FOLLOW_TREND?is_same_symbol(trend_following,TREND_BUY)?clrLightGreen:clrMistyRose:clrLightGray;
      ...
      if(count_heg_buy > 0)
         strCountB += "  Hg." + (string)count_heg_buy;

      strCountB += (count_buy >= START_EXIT_XL? " Ex":"");

      string strCountS = "";
      if(count_sel > 0)
         strCountS = "  L." + (string)count_sel + "S";
      if(count_heg_sel > 0)
         strCountB += "  Hg." + (string)count_heg_sel;

      strCountS += (count_sel >= START_EXIT_XL? " Ex":"");

      string stoc_main = "(" + format_double_to_string(AMP_TRADE,digits-1)
                         + "/" +  format_double_to_string(AMP_TP_Lx,digits-1)
...
if(false)
   {
    if(is_same_trend_dh1
       && is_same_symbol(trend_following,TREND_BUY) && is_same_symbol(total_comment_hedging_buy,MMDD_TREND_MA10_D1)==false && count_sel>0
       && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_ma(symbol,PERIOD_W1,10,1))
       && allow_trade_now_by_stoc(symbol,PERIOD_M5,MMDD_TREND_MA10_D1)
      )
      {
       count_heg_buy+=1;
       double TP_HEDING = ask+AMP_HEDGING;
       string comment_buy = create_comment(MMDD_TREND_MA10_D1,MASK_HEGING,count_heg_buy);

       double vol_hedgig = 0;
       if(vol_negertive_heg_sel>0)
         {
          double next_profit_hedgig = calcPotentialTradeProfit(symbol,TREND_SEL,ask,ask-AMP_HEDGING,vol_negertive_heg_sel);
          vol_hedgig = calc_volume_by_amp(symbol,AMP_HEDGING,MathAbs(profit_negertive_heg_sel)+MathAbs(next_profit_hedgig));
         }

       double next_profit_normal = calcPotentialTradeProfit(symbol,TREND_SEL,ask,ask-AMP_HEDGING,vol_negertive_sel);
       double vol = calc_volume_by_amp(symbol,AMP_HEDGING,MathAbs(profit_negertive_sel)+MathAbs(next_profit_normal));
       vol = NormalizeDouble((vol+vol_hedgig)*1.25,2);

       ClosePositivePosition(symbol,TREND_SEL);

       Open_Position(symbol,TREND_BUY,vol,0.0,TP_HEDING,comment_buy);

       ModifySL(symbol,MASK_NORMAL,TREND_SEL,TP_HEDING);
       ModifyTp2Entry(symbol,MASK_NORMAL,TREND_SEL);

       ModifySL(symbol,MASK_HEGING,TREND_SEL,TP_HEDING);
       ModifyTp2Entry(symbol,MASK_HEGING,TREND_SEL);

       return;
      }

    if(is_same_trend_dh1
       && is_same_symbol(trend_following,TREND_SEL) && is_same_symbol(total_comment_hedging_sel,MMDD_TREND_MA10_D1)==false && count_buy>0
       && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_ma(symbol,PERIOD_W1,10,1))
       && allow_trade_now_by_stoc(symbol,PERIOD_M5,MMDD_TREND_MA10_D1)
      )
      {
       ...
       string comment_sel = create_comment(MMDD_TREND_MA10_D1,MASK_HEGING,count_heg_sel);

       double vol_hedgig = 0;
       if(vol_negertive_heg_buy>0)
         {
          double next_profit_hedgig = calcPotentialTradeProfit(symbol,TREND_BUY,bid,bid+AMP_HEDGING,vol_negertive_heg_buy);
          vol_hedgig = calc_volume_by_amp(symbol,AMP_HEDGING,MathAbs(profit_negertive_heg_buy)+MathAbs(next_profit_hedgig));
         }

       double next_profit_normal = calcPotentialTradeProfit(symbol,TREND_BUY,bid,bid+AMP_HEDGING,vol_negertive_buy);
       double vol = calc_volume_by_amp(symbol,AMP_HEDGING,MathAbs(profit_negertive_buy)+MathAbs(next_profit_normal));
       vol = NormalizeDouble((vol+vol_hedgig)*1.25,2);

       ClosePositivePosition(symbol,TREND_BUY);

       Open_Position(symbol,TREND_SEL,vol,0.0,TP_HEDING,comment_sel);

       ModifySL(symbol,MASK_NORMAL,TREND_BUY,TP_HEDING);
       ModifyTp2Entry(symbol,MASK_NORMAL,TREND_BUY);

       ModifySL(symbol,MASK_HEGING,TREND_BUY,TP_HEDING);
       ModifyTp2Entry(symbol,MASK_HEGING,TREND_BUY);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Symbol(), symbol)
            && is_same_symbol(m_position.Comment(),BOT_SHORT_NM)
            && is_same_symbol(m_position.Comment(),comment))
            has_commets = true;
...
//|                                                                  |
//+------------------------------------------------------------------+
void ModifyTp2Entry(string symbol, string MASK, string TREND)
  {
   printf("ModifySL2Entry: " + symbol);
   int digits = (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Comment(),BOT_SHORT_NM) &&
            ...
            is_same_symbol(m_position.Comment(),MASK) &&
            is_same_symbol(m_position.Comment(),TREND))
           {
            bool is_same_sl = m_position.TakeProfit()==m_position.PriceOpen();
            if(is_same_sl==false)
               is_same_sl=format_double_to_string(m_position.PriceOpen(), digits-1) == format_double_to_string(m_position.TakeProfit(), digits-1) ;
            ...
            if(is_same_sl==false)
              {
               ...
               while(demm<5)
                 {
                  bool successful=m_trade.PositionModify(m_position.Ticket(),m_position.StopLoss(),m_position.PriceOpen());
                  if(successful)
                     break;
...
//|                                                                  |
//+------------------------------------------------------------------+
void ModifySL(string symbol, string MASK, string TREND, double sl_price)
  {
   printf("ModifySL: " + symbol);
   int digits = (int)SymbolInfoInteger(symbol, SYMBOL_DIGITS);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Comment(), BOT_SHORT_NM) &&
            ...
            is_same_symbol(m_position.Comment(),TREND))
           {
            bool is_same_sl = sl_price==m_position.StopLoss();
            if(is_same_sl==false)
               is_same_sl=format_double_to_string(sl_price, digits-1) == format_double_to_string(m_position.StopLoss(), digits-1) ;
...
//|                                                                  |
//+------------------------------------------------------------------+
void ModifyTp(string symbol, string MASK, string TREND, double tp_price)
  {
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
      if(m_order.SelectByIndex(i))
         if(is_same_symbol(m_order.Comment(), BOT_SHORT_NM) &&
            ...
            is_same_symbol(m_order.Comment(),MASK) &&
            is_same_symbol(m_order.Comment(),TREND))
           {
...
//|                                                                  |
//+------------------------------------------------------------------+
void get_trend_by_macd3c(string symbol, ENUM_TIMEFRAMES timeframe,string &trend,int &count,int &value
                         ,int fastEma=12,int slowEma=26,int macdEma=9)
  {
   trend="";
   count=0;
   value=0;
   ...
   CopyBuffer(m_handle_macd,0,0,50,M);
   CopyBuffer(m_handle_macd,1,0,50,S);

   if((M[1]>S[1]) && ((M[0]>M[1]) || (S[0]>S[1])))
      trend=TREND_BUY;
   ...
   for(int i=1;i<45;i++)
     {
      string trend_i = (M[i]>S[i])?TREND_BUY:TREND_SEL;
      if(trend_i!=trend)
        {
         string trend_i1 = (M[i+1]>S[i+1])?TREND_BUY:TREND_SEL;
         string trend_i2 = (M[i+2]>S[i+2])?TREND_BUY:TREND_SEL;
         string trend_i3 = (M[i+3]>S[i+3])?TREND_BUY:TREND_SEL;

         if(is_same_symbol(trend_i1+trend_i2+trend_i3,trend)==false)
           {
            count=i;
            break;
           }
...
//|                                                                  |
//+------------------------------------------------------------------+
void get_trend_by_stoc3c(string symbol, ENUM_TIMEFRAMES timeframe,string &trend_stoc_h27,int &count_stoc_h27,int &value_stoc_h27
                         , int inK=13, int inD=8, int inS=5, int candle_no=1)
  {
   trend_stoc_h27="";
   count_stoc_h27=0;
   value_stoc_h27=0;
   ...
   double bla_1 = K[candle_no];
   double red_1 = D[candle_no];
   trend_stoc_h27 = (bla_1>red_1)?TREND_BUY:TREND_SEL;
   value_stoc_h27=(int)(bla_1+red_1)/2;

   for(int i=candle_no;i<45;i++)
     {
      double bla_i = K[i];
      double red_i = D[i];
      string trend_stoc_h27i = (bla_i>red_i)?TREND_BUY:TREND_SEL;
      if(trend_stoc_h27i!=trend_stoc_h27)
        {
         string trend_stoc_h27_i1 = (K[i+1]>D[i+1])?TREND_BUY:TREND_SEL;
         string trend_stoc_h27_i2 = (K[i+2]>D[i+2])?TREND_BUY:TREND_SEL;
         string trend_stoc_h27_i3 = (K[i+3]>D[i+3])?TREND_BUY:TREND_SEL;

         if(is_same_symbol(trend_stoc_h27_i1+trend_stoc_h27_i2+trend_stoc_h27_i3,trend_stoc_h27)==false)
           {
            count_stoc_h27=i;
            break;
           }
...
string GetComments()
  {
   ...
   str_comments += "    Min: " + format_double_to_string(Risk_Min(), 2) + "$ Exit >= " + (string)START_EXIT_XL + "L";
   if(IS_ALLOW_TRAILING_STOP)
      str_comments += "    Train: " + format_double_to_string(AMP_TRAILING_STOP, digits);

   str_comments += "    TP_L1: " + format_double_to_string(AMP_TP_L1, digits)
                   + "    TP_Lx: " + format_double_to_string(AMP_TP_Lx, digits)
                   + "  SL_L1: " + format_double_to_string(AMP_SL_L1, digits)
                   + "  DCA: "+format_double_to_string(AMP_TRADE, digits)
   ...
   str_comments += "    m.A.Recover.B: " + AppendSpaces(format_double_to_string(GetGlobalVariable(MAX_AMP_SOLVE_BUY),2), 7, false);
   ...
   str_comments += "    m.A.Recover.S: " + AppendSpaces(format_double_to_string(GetGlobalVariable(MAX_AMP_SOLVE_SEL),2), 7, false);
