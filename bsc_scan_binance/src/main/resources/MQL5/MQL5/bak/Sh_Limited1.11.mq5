...
double FIBO = 1.0;
int START_EXIT_Eq = 12;
int START_EXIT_XL = 15;
double INIT_VOLUME=0.01;
double MULTI=2;
double AMP_TRADE = 3.0*MULTI;
double AMP_HEDGING = 10.0;
double AMP_TP_L1 = AMP_TRADE;
double AMP_TP_Lx = AMP_TRADE*3.618;
double AMP_TRAILING_STOP = 1.618*MULTI;
double AMP_STOP = AMP_TP_L1*START_EXIT_XL;
...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   string MMDD_TREND_MA10_D1 = get_mmdd_swith_trend(symbol,PERIOD_D1);

   string trend_following = " D10:" + MMDD_TREND_MA10_D1;

   if(is_same_symbol(MMDD_TREND_MA10_D1,TREND_SEL))
      IS_CYCLE_BUY=false;
   if(is_same_symbol(MMDD_TREND_MA10_D1,TREND_BUY))
      IS_CYCLE_SEL=false;
                  ...
                  if(is_same_symbol(m_position.TypeDescription(),TREND_BUY))
                    {
                     ...
                     //--------------------------------------TRAILING_STOP--------------------------------------
                     if(IS_ALLOW_TRAILING_STOP && m_position.Volume()>=INIT_VOLUME)
                       {
...
//---------------------------------------------------------------------------------------------------------
   ...
   if(true)
     {
      if(is_same_symbol(trend_following,TREND_BUY) && is_same_symbol(total_comment_hedging_buy,MMDD_TREND_MA10_D1)==false && count_sel>0
         && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_ma(symbol,PERIOD_W1,10,1))
         && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_stoc2(symbol,PERIOD_H1,26,12,9,1))
      ...
      if(is_same_symbol(trend_following,TREND_SEL) && is_same_symbol(total_comment_hedging_sel,MMDD_TREND_MA10_D1)==false && count_buy>0
         && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_ma(symbol,PERIOD_W1,10,1))
         && is_same_symbol(MMDD_TREND_MA10_D1,get_trend_by_stoc2(symbol,PERIOD_H1,26,12,9,1))
         && allow_trade_now_by_stoc(symbol,PERIOD_M5,MMDD_TREND_MA10_D1)
        )
...
//---------------------------------------------------------------------------------------------------------
   if(IS_CYCLE_BUY && (count_buy==0))
     {
      count_buy = 1;
      string comment_buy = create_comment(MASK_NORMAL,MMDD_TREND_MA10_D1,count_buy);
   ...
   if(IS_CYCLE_SEL && (count_sel==0))
     {
      count_sel = 1;
      string comment_sel = create_comment(MASK_NORMAL,MMDD_TREND_MA10_D1,count_sel);
   ...
   if(count_buy>0)
     {
      if((global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask)
         && IS_CYCLE_BUY && CanOpenDCA(symbol,PERIOD_M15,last_time_buy))
        {
         ...
         bool opened_buy = m_trade.BuyLimit(vol,bid-0.5,symbol,0.0,TP_BUY,0,0,comment_buy);

         if(opened_buy)
           {
            if(count_buy>=START_EXIT_XL)
              {
               count_buy+=1;
               vol = calc_volume_by_fibo_dca(symbol,count_buy,FIBO);
               comment_buy = create_comment(MASK_RECOVER,MMDD_TREND_MA10_D1,count_buy);
   ...
   if(count_sel>0)
     {
      if(IS_CYCLE_SEL && (global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid)
         && CanOpenDCA(symbol,PERIOD_M15,last_time_sel)) //
        {
         ...
         bool opened_sel = m_trade.SellLimit(vol,ask+0.5,symbol,0.0,TP_SEL,0,0,comment_sel);

         if(opened_sel)
           {
            if(count_sel>=START_EXIT_XL)
              {
               count_sel+=1;
               vol = calc_volume_by_fibo_dca(symbol,count_sel,FIBO);
               comment_sel = create_comment(MASK_RECOVER,MMDD_TREND_MA10_D1,count_sel);
