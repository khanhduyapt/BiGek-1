...
double AMP_TRADE=6.0;
double AMP_HEDGING=6.0;
double AMP_TP_L1=AMP_TRADE;
double AMP_TP_Lx=AMP_TRADE*1.618;
...
//---------------------------------------------------------------------------------------------------------
   ...
   double TP_BUY_Lx=global_max_entry_buy+AMP_TP_L1;
   double TP_SEL_Lx=global_min_entry_sel-AMP_TP_L1;
//---------------------------------------------------------------------------------------------------------
   ...
   if(count_buy>0)
     {
      if((global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask)
         && IS_CYCLE_BUY && CanOpenDCA(symbol,PERIOD_M15,last_time_buy))
        {
         ...
         bool opened_buy=m_trade.BuyLimit(vol,bid-0.5,symbol,0.0,TP_BUY_Lx,0,0,comment_buy);
         if(opened_buy)
            return;
        }
      ...
      if(potential_profit_buy<=risk_1L)
        {
         if(TP_BUY_Lx==0)
            TP_BUY_Lx=global_max_entry_buy+AMP_TP_L1;

         ModifyTp(symbol,TREND_BUY,TREND_BUY,TP_BUY_Lx+0.5);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   ...
   if(count_sel>0)
     {
      if(IS_CYCLE_SEL && (global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid)
         && CanOpenDCA(symbol,PERIOD_M15,last_time_sel)) //
        {
         ...
         bool opened_sel=m_trade.SellLimit(vol,ask+0.5,symbol,0.0,TP_SEL_Lx,0,0,comment_sel);
   ...
   if(allow_trade_now_by_stoc(symbol,PERIOD_H1,TREND_NEW) && allow_trade_now_by_stoc(symbol,PERIOD_M5,TREND_NEW))
     {
      ...
      double vol_hedging=NormalizeDouble((volume+vol_recover)*1.25,2);
      ...
      if(is_same_symbol(TREND_NEW,TREND_BUY))
        {
         ...
         bool result=m_trade.Buy(vol_hedging,symbol,0.0,price_open,TP_HEDING,comment_buy);
         if(result)
           {
            m_trade.PositionModify(ticket,TP_HEDING,price_open);
            if(result)
               return true;
           }
        }
      ...
      if(is_same_symbol(TREND_NEW,TREND_SEL))
        {
         ...
         bool result=m_trade.Sell(vol_hedging,symbol,0.0,price_open,TP_HEDING,comment_sel);
         if(result)
           {
            m_trade.PositionModify(ticket,TP_HEDING,price_open);
            if(result)
