...
datetime TIME_OF_ONE_05_CANDLE=300;
...
int OnInit()
  {
   ...
   double low_price,hig_price;
   is_allow_trade_by_macd_extremes(symbol,Period(),TREND_BUY,low_price,hig_price);
...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   get_arr_heiken(symbol,PERIOD_M5,arrHeiken_m5,10,true);

   string FIND_TREND=arrHeiken_h1[1].trend_by_ma10+arrHeiken_h1[0].trend_by_ma10+arrHeiken_m5[1].trend_by_ma10;
   ...
   bool is_pass_wait_buy=(low_price>0) && (cur_price<low_price);
   bool is_pass_wait_sel=(hig_price>0) && (cur_price>hig_price);
...
//---------------------------------------------------------------------------------------------------------
   if(IS_CYCLE_BUY && (count_buy==0) && is_pass_wait_sel==false)
     {
      count_buy=1;
   ...
   if(IS_CYCLE_SEL && (count_sel==0) && is_pass_wait_buy==false)
     {
      count_sel=1;
...
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      bool is_append=(global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask) && PassedWait(TREND_BUY,last_time_buy,PERIOD_M5);
      if(is_append)
        {
         // CanOpenDCA(symbol,PERIOD_H4,last_time_buy);
         //if(is_pass_wait_buy)
           {
            count_buy+=1;
...
//---------------------------------------------------------------------------------------------------------
   if(count_sel>0)
     {
      bool is_append=(global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid) && PassedWait(TREND_SEL,last_time_sel,PERIOD_M5);
      if(is_append)
        {
         // CanOpenDCA(symbol,PERIOD_H4,last_time_sel);
         //if(is_pass_wait_sel)
           {
            count_sel+=1;
            string mask=MASK_NORMAL;
   ...
   if(ALLOW_DRAW_BUTONS && is_cur_tab)
     {
      ...
      if(count_buy>0)
         lblProfitBuy=(count_buy>1?format_double_to_string(range_buy,digits-1):"")
                      +" "+DoubleToString(vol_buy,2)+".lot "+DoubleToString(profit_buy,2)+"$";

      string lblProfitSel="";
      if(count_sel>0)
         lblProfitSel=(count_sel>1?format_double_to_string(range_sel,digits-1):"")
                      +" "+DoubleToString(vol_sel,2)+".lot "+DoubleToString(profit_sel,2)+"$";
   ...
   if(found_positive && found_negative)
     {
      ...
      create_label_simple(TF+"MACD_B",""+StringSubstr(TF,0,2)+ " "+DoubleToString(amp_wave,digits),low_price-candle_heigh,clrBlack,lowtime);
      ...
      create_label_simple(TF+"MACD_S",""+StringSubstr(TF,0,2)+ " "+DoubleToString(amp_wave,digits),hig_price+candle_heigh,clrBlack,higtime);
...
void DrawFibonacciRetracement(string name,datetime time1,double price1,datetime time2,double price2,color clrLevelColor=clrDimGray)
  {
   ...
   create_trend_line(name+"_0",time_min,MathMin(price1,price2)-amp_fibo*0,TimeCurrent(),MathMin(price1,price2)-amp_fibo*0,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"_1",time_min,MathMax(price1,price2)+amp_fibo*0,TimeCurrent(),MathMax(price1,price2)+amp_fibo*0,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"-1",time_max,MathMin(price1,price2)-amp_fibo*1,TimeCurrent(),MathMin(price1,price2)-amp_fibo*1,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"+1",time_max,MathMax(price1,price2)+amp_fibo*1,TimeCurrent(),MathMax(price1,price2)+amp_fibo*1,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"-2",time_max,MathMin(price1,price2)-amp_fibo*2,TimeCurrent(),MathMin(price1,price2)-amp_fibo*2,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"+2",time_max,MathMax(price1,price2)+amp_fibo*2,TimeCurrent(),MathMax(price1,price2)+amp_fibo*2,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"-3",time_max,MathMin(price1,price2)-amp_fibo*3,TimeCurrent(),MathMin(price1,price2)-amp_fibo*3,clrBlack,STYLE_SOLID,1,false,true,true,false);
   create_trend_line(name+"+3",time_max,MathMax(price1,price2)+amp_fibo*3,TimeCurrent(),MathMax(price1,price2)+amp_fibo*3,clrBlack,STYLE_SOLID,1,false,true,true,false);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
//|                                                                  |
//+------------------------------------------------------------------+
bool PassedWait(string TREND, datetime last_order_time, ENUM_TIMEFRAMES PERIOD)
  {
   int wait=(int)TIME_OF_ONE_H1_CANDLE;
   if(PERIOD==PERIOD_H4)
      wait=(int)TIME_OF_ONE_H4_CANDLE;
   if(PERIOD==PERIOD_M15)
      wait=(int)TIME_OF_ONE_15_CANDLE;   //Đã chờ 15 phút (15*60=900 giây)
   if(PERIOD==PERIOD_M5)
      wait=(int)TIME_OF_ONE_05_CANDLE;
   ...
   if(TREND==TREND_BUY)
      createButton("WaitedB","(B) "+hhmm,10,y_ref_btn+30,68,20,clrBlack,(seconds_left>=wait)?clrActiveBtn:clrLightGray,7);
   if(TREND==TREND_SEL)
      createButton("WaitedS","(S) "+hhmm,10,y_ref_btn+60,68,20,clrBlack,(seconds_left>=wait)?clrActiveBtn:clrLightGray,7);
