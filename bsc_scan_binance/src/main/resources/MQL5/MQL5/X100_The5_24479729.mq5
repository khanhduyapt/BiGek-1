...
#define GLOBAL_LINE_TIMER_ "GLOBAL_LINE_TIMER_"
...
#define BtnSeqD1_                   "BtnSeqD1_"
#define BtnSeqH4_                   "BtnSeqH4_"
#define BtnSeqH1_                   "BtnSeqH1_"
#define BtnSeqM15_                  "BtnSeqM15_"
#define BtnSLByMa50H4_              "BtnSLByMa50H4_"
#define BtnMsgSeqH1_                "BtnMsgSeqH1_"
#define BtnClearMsgSeqH1            "BtnClearMsgSeqH1"
#define BtnClearMessageSL           "BtnClearMessageSL"
#define BtnClearMessageH4           "BtnClearMessageH4"
#define BtnClearMessageH1           "BtnClearMessageH1"
#define BtnClearMessageM15          "BtnClearMessageM15"
#define BtnClearMessageD1           "BtnClearMessageD1"
...
const string FILE_MSG_LIST_M15= "_messages_15.txt";
const string FILE_MSG_LIST_H1 = "_messages_h1.txt";
const string FILE_MSG_LIST_H4 = "_messages_h4.txt";
const string FILE_MSG_LIST_D1 = "_messages_d1.txt";
const string FILE_MSG_LIST_SL = "_messages_sl.txt";
const string FILE_MSG_SEQ_H1  = "_messages_seq_h1.txt";
...
int OnInit()
  {
   ...
   if(Period()<PERIOD_D1)
     {
      Draw_Ma(cur_symbol,PERIOD_W1, 10,20,20);
      Draw_Ma(cur_symbol,PERIOD_D1, 10,15,50);
      //DrawFiboTimeZone52H4(cur_symbol);
     }
   if(Period()==PERIOD_D1)
     {
      //Draw_Fibo(cur_symbol,PERIOD_D1);
      Draw_Ma(cur_symbol,PERIOD_MN1,10,20, 50);
      Draw_Ma(cur_symbol,PERIOD_W1, 10,15,100);
      Draw_Ma(cur_symbol,PERIOD_W1,  5, 7,100);
     }
   if(Period()>PERIOD_D1)
      ObjectsDeleteAll(0);
...
void OnTick()
  {
   ...
   if(EQUITY>=10805)
     {
      if(is_same_symbol(AccountInfoString(ACCOUNT_NAME),"10K"))
        {
               ...
               if(is_same_symbol(m_position.Symbol(),temp_symbol))
                 {
                  ...
                  if(is_exit_all)
                    {
                     m_trade.PositionClose(m_position.Ticket());
                     SendTelegramMessage(temp_symbol,TREND_TYPE,"(EXTI)"+msg);
                  ...
                  //-------------------------------------------------------------------
                  if(temp_profit>1)
                     if(is_allow_trade_by_macd_extremes(temp_symbol,PERIOD_H1,trend_reverse))
                  ...
                  if(PassedWaitHours(m_position.Time(), 12) && (temp_profit>1))
                    {
                     ...
                     //-------------------------------------------------------------------
                     if(arrHeiken_H4[0].trend_by_ma10==trend_reverse &&
                     ...
                     if(temp_profit>risk_1L/5)
                       {
                        if(trend_reverse==arrHeiken_H4[0].trend_heiken &&
                           ...
                           trend_reverse==arrHeiken_H4[1].trend_by_ma10)
                          {
                           ...
                           PushMessage(msg_OK,FILE_MSG_LIST_SL);
                           reload=true;
                           continue;
                          }
                       }
   ...
   for(int index = 0; index < size; index++)
     {
      ...
      string trend_by_ma20_h4=get_trend_by_ma(symbol,PERIOD_H4,10,0);
      if(trend_by_ma10_w1==trend_by_ma10_d1)
        {
         lblBtn10="WD"+(trend_by_ma10_w1==trend_by_ma20_h4?"H4":"")+" "+trend_by_ma10_w1+" "+lblBtn10;
...
//--------------------------------------------------------------------------------------------
   ...
   if(Period()<PERIOD_D1)
     {
      ...
      Draw_MACD_Extremes(symbol,PERIOD_D1,50,true, 1,STYLE_SOLID);
   ...
   CreateMessagesBtn(BtnSeqM15_);
   CreateMessagesBtn(BtnSeqH1_);
   CreateMessagesBtn(BtnSeqH4_);
   CreateMessagesBtn(BtnSLByMa50H4_);
   CreateMessagesBtn(BtnMsgSeqH1_);
   ...
   string setup_tp_when=(string)GetGlobalVariable(BtnTPWhenMaCross+symbol);
   string tp_H1Ma20="[TP] When (H1) Hei.Ma20"+(is_same_symbol(setup_tp_when,HeivsMa20_BUY+"1")?" "+TREND_BUY:"")+(is_same_symbol(setup_tp_when,HeivsMa20_SEL+"1")?" "+TREND_SEL:"");
   string tp_H4Ma20="[TP] When (H4) Hei.Ma20"+(is_same_symbol(setup_tp_when,HeivsMa20_BUY+"4")?" "+TREND_BUY:"")+(is_same_symbol(setup_tp_when,HeivsMa20_SEL+"4")?" "+TREND_SEL:"");
   createButton(BtnTPWhenMaCross+"_H1Ma20",tp_H1Ma20,col_7,init_y+ 5,220,25,clrBlack,is_same_symbol(setup_tp_when,HeivsMa20_BUY+"1")?clrActiveBtn:is_same_symbol(setup_tp_when,HeivsMa20_SEL+"1")?clrActiveSell:is_period_h1?clrWhite:clrLightGray,7,STOC_WINDOW);
   createButton(BtnTPWhenMaCross+"_H4Ma20",tp_H4Ma20,col_7,init_y+45,220,25,clrBlack,is_same_symbol(setup_tp_when,HeivsMa20_BUY+"4")?clrActiveBtn:is_same_symbol(setup_tp_when,HeivsMa20_SEL+"4")?clrActiveSell:is_period_h4?clrWhite:clrLightGray,7,STOC_WINDOW);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void init_sl_tp_trendline(bool is_reset_sl,bool reverse_ma10d1=false)
  {
   ...
   bool is_cur_tab=is_same_symbol(symbol,Symbol());
   double line_timer=GetGlobalVariable(GLOBAL_LINE_TIMER_+symbol);

   string total_comments="";
   string strBSL=CountBSL(symbol,total_comments,true);//Alert(total_comments);

   if(line_timer<=0)
     {
      line_timer=(double)(TimeCurrent()+TIME_OF_ONE_H4_CANDLE);
      SetGlobalVariable(GLOBAL_LINE_TIMER_+symbol,line_timer);
     }
...
//double volume_w1=calc_volume_by_amp(symbol,amp_w1,risk);
   ...
   create_dragable_trendline(GLOBAL_VAR_SL,clrRed,  SL,STYLE_SOLID,3);
   create_dragable_trendline(GLOBAL_VAR_LM,clrGreen,LM,STYLE_SOLID,3);

   create_dragable_trendline(LINE_RR_11,clrBlack,rr11,STYLE_DASHDOTDOT);
   create_dragable_trendline(LINE_RR_12,clrBlack,rr12,STYLE_DASHDOTDOT);
   create_dragable_trendline(LINE_RR_13,clrBlue, rr13,STYLE_DASHDOTDOT);
   ...
   if(ChartTimePriceToXY(0,0,time,LM,x,y_start))
     {
      ...
      if(strBSL!="")
         createButton(BtnReSetTPEntry,"Tp.Entry",start_group_reverse-500,y_start-10,60,20,clrBlack,clrWhite);
      ...
      createButton(BtnSetPriceLimit+"(h4)10","(h4)10",x_start-15,y_start+00,50,18,clrBlack,is_same_symbol(trend,TREND_BUY)?clrActiveBtn:clrActiveSell);
      createButton(BtnSetPriceLimit+"(h4)20","(h4)20",x_start+40,y_start+00,50,18,clrBlack,is_same_symbol(trend,TREND_BUY)?clrActiveBtn:clrActiveSell);
      createButton(BtnSetPriceLimit+"(h4)50","(h4)50",x_start+95,y_start+00,50,18,clrBlack,is_same_symbol(trend,TREND_BUY)?clrActiveBtn:clrActiveSell);
     }
   ...
   if(strBSL!="")
      createButton(BtnCloseSymbol,"Close "+symbol+" "+strBSL,chart_width/2-310,chart_heigh-35,200,30,clrBlack,clrLightGray,7);
         ...
         if(is_allow_trade_by_macd_extremes(symbol,PERIOD_M5,trend_wait_rr11))
           {
            ...
            if(market_ok)
            ...
            else
              {
               ...
               PushMessage(msg_NG,FILE_MSG_LIST_H4);
              }
           }
        }
      //----------------------------------------------------------------------------------------------------
            ...
            if(is_same_symbol(trend_by_seq_102050,wait_trend_m5))
              {
               ...
               PushMessage(msg,FILE_MSG_LIST_M15);
...
void CheckTrendlineUpdates(bool re_draw=false)
  {
   ...
   UpdateGlobalTimeOnChange(symbol);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void UpdateGlobalTimeOnChange(string symbol)
  {
   datetime base_time = (datetime)GetGlobalVariable(GLOBAL_LINE_TIMER_+symbol);
   if(base_time<=0)
      base_time=TimeCurrent();

   datetime new_time = (datetime)ObjectGetInteger(0, GLOBAL_LINE_TIMER_, OBJPROP_TIME);

   if(base_time != new_time)
     {
      SetGlobalVariable(GLOBAL_LINE_TIMER_+symbol, new_time);
      DrawFiboTimeZone52H4(symbol);
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void DrawFiboTimeZone52H4(string symbol)
  {
   int width=3;
   if(Period()>PERIOD_H4)
     {
   ...
   if(Period()<PERIOD_D1)
     {
      Print(get_vntime()+"DrawFiboTimeZone52H4");
      string fibo_name = FIBO_TIME_ZONE_+symbol;
      string global_name = GLOBAL_LINE_TIMER_+symbol;
      datetime base_time = (datetime)GetGlobalVariable(global_name);

      create_dragable_vertical_line(GLOBAL_LINE_TIMER_,base_time,clrBlue,STYLE_SOLID,width);

      if(!ObjectCreate(0, fibo_name, OBJ_FIBOTIMES, 0, base_time, 0))
        {
      ...
      ObjectCreate(0, fibo_4_seasons, OBJ_FIBOTIMES, 0, base_time, 0, base_time + interval, 0);
      ...
      double levels_4seasons[] = {0,0.236, 0.5, 0.764, 1
                                  ,-0.236,-0.5,-0.764,-1
                                 };
      ...
      for(int i = 0; i < levels_4seasons_count; i++)
        {
         ...
         ObjectSetInteger(0, fibo_4_seasons, OBJPROP_LEVELCOLOR, i, clrFireBrick);
         ObjectSetInteger(0, fibo_4_seasons, OBJPROP_LEVELSTYLE, i, STYLE_SOLID);
      ...
      ObjectCreate(0, fibo_name, OBJ_FIBOTIMES, 0, base_time, 0, base_time + interval, 0);
...
void Draw_Fibo(string symbol, ENUM_TIMEFRAMES TIMEFRAME)
  {
   ...
   DeleteAllObjectsWithPrefix("TimeZone");
   DeleteAllObjectsWithPrefix("_Fibo_Fan_");
   ...
   ObjectDelete(0, "_Fibo_Fan_S");
   ObjectDelete(0, "_Fibo_Fan_B");
   if(Period()<=PERIOD_D1)
     {
      ...
      if(MathAbs(price1-price2)>amp_w1)
        {
         DrawFibonacciTimeZones(BOT_SHORT_NM+"TimeZone", date2, date1, price1);
         DrawFibonacciFan(BOT_SHORT_NM+"_Fibo_Fan_S", date2, MathMin(price1, price2), date1, MathMax(price1, price2), TREND_SEL);
         DrawFibonacciFan(BOT_SHORT_NM+"_Fibo_Fan_B", date2, MathMax(price1, price2), date1, MathMin(price1, price2), TREND_BUY);
        }
     }
  }
//+------------------------------------------------------------------+
            ...
            if(histogram>0)
              {
               ...
               if(timeframe==PERIOD_D1)
                  create_trend_line(TF+"MACD_LOW_D" + appendZero100(lowest_negative_index),time,low,time+1,low,clrDodgerBlue,STYLE_SOLID,25); //
            ...
            if(histogram<0)
              {
               ...
               if(timeframe==PERIOD_D1)
                  create_trend_line(TF+"MACD_HIG_D" + appendZero100(highest_positive_index),time,hig,time+1,hig,clrTomato,STYLE_SOLID,25); //
...
public:
   ...
   double            ma05;
                     ...
                     CandleData()
     {
      ...
      double lowest_,  double higest_,bool allow_trade_now_by_seq_051020_,bool allow_trade_now_by_seq_102050_,double ma20_,int count_ma20_,string trend_by_ma20_,double ma05_)
     {
      time=t;
      open=o;
      high=h;
...
//|                                                                  |
//+------------------------------------------------------------------+
void Draw_Ma(string symbol,ENUM_TIMEFRAMES timeframe, int ma_length, int width_ma10,int length=150,color clrColorBuy=clrPaleTurquoise, color clrSell=clrLightPink)
  {
   if(is_same_symbol(symbol,Symbol())==false)
      return;
   ...
   for(int i = 0; i<loop; i++)
     {
      ...
      double ma10_0=cal_MA(closePrices,ma_length,i);
      double ma10_1=cal_MA(closePrices,ma_length,i+1);

      color clrColorW = closePrices[i]>ma10_0?clrColorBuy:clrSell;
      if(IS_MONOCHROME_MODE)
         clrColorW=clrLightGray;

      create_trend_line("Draw_Ma"+IntegerToString(width_ma10)+TF+"_"+append1Zero(i+1)+"_"+append1Zero(i),time1,ma10_1,time0,ma10_0,clrColorW,STYLE_SOLID,width_ma10);
      if(i==0)
         create_label("Draw_Ma"+IntegerToString(width_ma10)+TF,TimeCurrent(),ma10_0,"   Ma"+IntegerToString(ma_length)+"_"+TF+" ("+DoubleToString(ma10_0,digits-1)+")");
     }
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
void Set_LM_SL(string symbol, string trend_by_ma10_d1)
  {
   ...
   double LM=is_same_symbol(trend_by_ma10_d1,TREND_BUY)?MathMax(arrHeiken_cur[1].ma10,arrHeiken_cur[0].ma10):MathMin(arrHeiken_cur[1].ma10,arrHeiken_cur[0].ma10);


   double amp_sl=MathMin(MathMax(max_candle,amp_h4),amp_d1);
...
void OnChartEvent(const int     id,      // event ID
                 ...
                 )
  {
   if(id==CHARTEVENT_OBJECT_DRAG)
     {
      if(is_same_symbol(sparam,GLOBAL_VAR_SL) || is_same_symbol(sparam,GLOBAL_VAR_LM) || is_same_symbol(sparam,GLOBAL_LINE_TIMER_))
        {
   ...
   if(id==CHARTEVENT_OBJECT_CLICK)
     {
         ...
         if(is_same_symbol(sparam,BtnFindBuy))
           {
            ...
            SetGlobalVariable(GLOBAL_VAR_SL+symbol,ask-amp_d1);
           }
         ...
         if(is_same_symbol(sparam,BtnFindSel))
           {
            trend=TREND_SEL;
            SetGlobalVariable(GLOBAL_VAR_LM+symbol,bid);
            SetGlobalVariable(GLOBAL_VAR_SL+symbol,bid+amp_d1);
           }
      ...
      if(is_same_symbol(sparam,BtnTPWhenMaCross))
        {
         string buttonLabel = ObjectGetString(0,sparam,OBJPROP_TEXT);
         ...
         string _H1Ma20=ObjectGetString(0,BtnTPWhenMaCross+"_H1Ma20",OBJPROP_TEXT);
         string _H4Ma20=ObjectGetString(0,BtnTPWhenMaCross+"_H4Ma20",OBJPROP_TEXT);

         string setup_tp_when="7";
         setup_tp_when+=is_same_symbol(_H1Ma20,TREND_BUY)?HeivsMa20_BUY+"1":"";
         setup_tp_when+=is_same_symbol(_H1Ma20,TREND_SEL)?HeivsMa20_SEL+"1":"";

         setup_tp_when+=is_same_symbol(_H4Ma20,TREND_BUY)?HeivsMa20_BUY+"4":"";
         setup_tp_when+=is_same_symbol(_H4Ma20,TREND_SEL)?HeivsMa20_SEL+"4":"";

         SetGlobalVariable(BtnTPWhenMaCross+symbol,(double)setup_tp_when);
      ...
      if(is_same_symbol(sparam,BtnSeqD1_) ||
         is_same_symbol(sparam,BtnMsgSeqH1_) ||
         is_same_symbol(sparam,BtnSLByMa50H4_) ||
         is_same_symbol(sparam,BtnSeqH4_) ||
         is_same_symbol(sparam,BtnSeqH1_) ||
         is_same_symbol(sparam,BtnSeqM15_))
        {
         string buttonLabel = ObjectGetString(0,sparam,OBJPROP_TEXT);
         Print("The lparam=",lparam," dparam=",dparam," sparam=",sparam," buttonLabel=",buttonLabel," was clicked");

         int intPeriod = (int)GetGlobalVariable(BtnOptionPeriod);//PERIOD_H4;
         //if(is_same_symbol(sparam,BtnSeqH4_))
         //   intPeriod = PERIOD_H4;
         //if(is_same_symbol(sparam,BtnSeqH1_) || is_same_symbol(sparam,BtnMsgSeqH1_))
         //   intPeriod = PERIOD_H1;
         //if(is_same_symbol(sparam,BtnSeqM15_))
      ...
      if(is_same_symbol(sparam,BtnClearMessageD1) ||
         is_same_symbol(sparam,BtnClearMsgSeqH1) ||
         is_same_symbol(sparam,BtnClearMessageSL) ||
         is_same_symbol(sparam,BtnClearMessageH4) ||
         is_same_symbol(sparam,BtnClearMessageH1) ||
         is_same_symbol(sparam,BtnClearMessageM15))
        {
         ...
         if(is_same_symbol(sparam,BtnClearMessageD1))
           {
            WriteFileContent(FILE_MSG_LIST_D1,"");
            CreateMessagesBtn(BtnSeqD1_);
           }

         if(is_same_symbol(sparam,BtnClearMsgSeqH1))
           {
            WriteFileContent(FILE_MSG_SEQ_H1,"");
            CreateMessagesBtn(BtnMsgSeqH1_);
           }

         if(is_same_symbol(sparam,BtnClearMessageSL))
           {
            WriteFileContent(FILE_MSG_LIST_SL,"");
            CreateMessagesBtn(BtnSLByMa50H4_);
           }

         if(is_same_symbol(sparam,BtnClearMessageH4))
           {
            int size = getArraySymbolsSize();
            for(int index = 0; index < size; index++)
              {
            ...
            WriteFileContent(FILE_MSG_LIST_H4,"");
            CreateMessagesBtn(BtnSeqH4_);
           }

         if(is_same_symbol(sparam,BtnClearMessageH1))
           {
            int size = getArraySymbolsSize();
            for(int index = 0; index < size; index++)
              {
            ...
            WriteFileContent(FILE_MSG_LIST_H1,"");
            CreateMessagesBtn(BtnSeqH1_);
           }

         if(is_same_symbol(sparam,BtnClearMessageM15))
           {
            int size = getArraySymbolsSize();
            for(int index = 0; index < size; index++)
              {
            ...
            WriteFileContent(FILE_MSG_LIST_M15,"");
            CreateMessagesBtn(BtnSeqM15_);
           }
      ...
      if(is_same_symbol(sparam,BtnOpen1L))
        {
         ...
         bool is_stop_loss_now=is_stop_loss_now_by_WDH(symbol,trend_now);
         if(is_stop_loss_now)
           {
            string msg=get_vnhour()+" "+symbol+" H4 (WDH) NotAllow "+trend_now;
            Alert(msg);
            return;
           }
         ...
         if(result==IDYES)
           {
            if(is_same_symbol(trend,TREND_BUY) || is_same_symbol(trend,TREND_SEL))
              {
               //if(PositionsTotal()>=MAXIMUM_OPENING)
               //  {
               //   //volume=calc_volume_by_amp(symbol,amp_sl,risk/2);
               //   Alert("Maximum "+(string)MAXIMUM_OPENING+"L");
               //   return;
               //  }
              }

            bool market_ok = Open_Position(symbol,trend,volume,0.0,LM,TP,comment_market,comment_market);
         ...
         if(result==IDNO)
           {
            ...
            bool market_ok = Open_Position(symbol,trend_now,volume,0.0,LM,TP,comment_market,comment_market);
            ...
            if(result==IDYES)
              {
               ...
               //if(is_same_symbol(sparam,BtnOpenRR11) && is_same_symbol(comment_market,MASK_ALLOW_TRIPPLE))
               //  {
               //   string trend_limit=is_same_symbol(trend,TREND_BUY)?TREND_LIMIT_BUY:TREND_LIMIT_SEL;
               //   double price_limit=is_same_symbol(trend,TREND_BUY)?SL-amp_sl:SL+amp_sl;
               //   comment_market=create_comment_timeframe(trend,0);
               //   strRR=trend+"_"+appendZero100(0);
               //
               //   Open_Position(symbol,trend_limit,volume,0.0,price_limit,TP,comment_market,strRR);
               //  }
            ...
            while(demm<5)
              {
               ...
               //printf("ModifyTpEntry "+(string)demm);
...
void CreateMessagesBtn(string BtnSeq___)
  {
   string BtnClearMessage=BtnClearMessageM15;
   string FILE_NAME_MSG_LIST=FILE_MSG_LIST_M15;
   int x_position=1050; //+260
   int y_position=70;
   string prifix="M15";

   if(BtnSeq___==BtnSeqH4_)
     {
      BtnClearMessage=BtnClearMessageH4;
      FILE_NAME_MSG_LIST=FILE_MSG_LIST_H4;
      x_position=10;
      prifix="H4";
     }

   if(BtnSeq___==BtnSeqH1_)
     {
      BtnClearMessage=BtnClearMessageH1;
      FILE_NAME_MSG_LIST=FILE_MSG_LIST_H1;
      x_position=270;
      prifix="H1";
     }

   if(BtnSeq___==BtnMsgSeqH1_)
     {
      BtnClearMessage=BtnClearMsgSeqH1;
      FILE_NAME_MSG_LIST=FILE_MSG_SEQ_H1;
      x_position=530;
      prifix="SeqH1";
     }

   if(BtnSeq___==BtnSLByMa50H4_)
     {
      BtnClearMessage=BtnClearMessageSL;
      FILE_NAME_MSG_LIST=FILE_MSG_LIST_SL;
      x_position=790;
      prifix="Ma50H4";
     }

   if(BtnSeq___==BtnSeqD1_)
     {
      BtnClearMessage=BtnClearMessageD1;
      FILE_NAME_MSG_LIST=FILE_MSG_LIST_D1;
      x_position=10;
      y_position=350;
      prifix="Ma10D1";
     }
            ...
            if(is_cur_tab)
              {
               ...
               if(MathAbs(temp_profit)>0)
                 {
                  double bid = SymbolInfoDouble(symbol,SYMBOL_BID);
                  create_label_simple("Profit"+(string)m_position.Ticket(),"P: "+DoubleToString(temp_profit,1)+"$",bid,temp_profit<0?clrRed:clrBlue,draw_time);
...
//|                                                                  |
//+------------------------------------------------------------------+
bool allow_trade_now_by_stoc(string symbol,ENUM_TIMEFRAMES timeframe,string FIND_TREND,int inK=21,int inD=7,int inS=7,int candle_no=0)
  {
   int handle_iStochastic=iStochastic(symbol,timeframe,inK,inD,inS,MODE_SMA,STO_LOWHIGH);
   if(handle_iStochastic==INVALID_HANDLE)
      return false;
...
bool is_exit_trade_time()
  {
   ...
   if(day_of_week==WEDNESDAY && 16 <= currentHour)
      return true;
      ...
      for(int index=loop; index>=0; index--)
        {
         ...
         CandleData candle_x(cur_cancle.time,cur_cancle.open,cur_cancle.high,cur_cancle.low,cur_cancle.close,cur_cancle.trend_heiken
                             ...
                             ,ma20,count_ma20,trend_by_ma20,ma05);
...
string create_comment(string TRADER, string TRADING_TREND, int L)
  {
   string result = get_current_timeframe_to_string()+ TRADER + TRADING_TREND + "_" + appendZero100(L);
