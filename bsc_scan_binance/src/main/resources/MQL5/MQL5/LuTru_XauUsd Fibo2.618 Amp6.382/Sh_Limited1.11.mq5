...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   get_arr_heiken(symbol, PERIOD_D1, arrHeiken_D1,15,true);


   string trend_ma10_d1 = get_trend_by_ma(symbol,PERIOD_D1,10,1);
   string trend_following = " D10:" + trend_ma10_d1;

   if(is_same_symbol(trend_ma10_d1,TREND_SEL))
      IS_CYCLE_BUY=false;
   if(is_same_symbol(trend_ma10_d1,TREND_BUY))
      IS_CYCLE_SEL=false;
...
//---------------------------------------------------------------------------------------------------------
   ...
   if(true)
     {
      if(is_same_symbol(trend_following,TREND_BUY) && count_heg_buy==0 && count_sel>0)
        {
         count_heg_buy+=1;
         string comment_buy = create_comment(TREND_BUY,MASK_HEGING,count_heg_buy);
         double next_profit = calcPotentialTradeProfit(symbol,TREND_SEL,ask,ask-AMP_TP_L1,vol_negertive_sel);
         double vol = calc_volume_by_amp(symbol,AMP_TP_L1,MathAbs(profit_negertive_sel)+MathAbs(next_profit));
         vol = NormalizeDouble(vol*1.25,2);

         ClosePositivePosition(symbol,TREND_SEL);
         Open_Position(symbol,TREND_BUY,vol,0.0,TP_BUY_L1,comment_buy);
         ModifySL(symbol,TREND_SEL,TP_BUY_L1);
         ModifyTp2Entry(symbol,TREND_SEL,TREND_SEL);
         return;
        }

      if(is_same_symbol(trend_following,TREND_SEL) && count_heg_sel==0 && count_buy>0)
        {
         count_heg_sel+=1;
         string comment_sel = create_comment(TREND_SEL,MASK_HEGING,count_heg_sel);
         double next_profit = calcPotentialTradeProfit(symbol,TREND_BUY,bid,bid+AMP_TP_L1,vol_negertive_buy);
         double vol = calc_volume_by_amp(symbol,AMP_TP_L1,MathAbs(profit_negertive_buy)+MathAbs(next_profit));
         vol = NormalizeDouble(vol*1.25,2);

         ClosePositivePosition(symbol,TREND_BUY);
         Open_Position(symbol,TREND_SEL,vol,0.0,TP_SEL_L1,comment_sel);
         ModifySL(symbol,TREND_BUY,TP_SEL_L1);
         ModifyTp2Entry(symbol,TREND_BUY,TREND_BUY);
         return;
        }

      if(is_moved_2_entry_buy==false && count_heg_buy>0 && count_sel==0)
        {
         ModifyTp2Entry(symbol,TREND_BUY,MASK_HEGING);
        }

      if(is_moved_2_entry_sel==false && count_heg_sel>0 && count_buy==0)
        {
         ModifyTp2Entry(symbol,TREND_SEL,MASK_HEGING);
        }
     }
//---------------------------------------------------------------------------------------------------------
      ...
      if((global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask)
         && IS_CYCLE_BUY && CanOpenDCA(symbol,PERIOD_M15,last_time_buy))
        {
         ...
         bool opened_buy = //Open_Position(symbol,TREND_BUY,vol,0.0,0.0,comment_buy);
            m_trade.BuyLimit(vol,bid-0.5,symbol,0.0,TP_BUY,0,0,comment_buy);
      ...
      if(IS_CYCLE_SEL && (global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid)
         && CanOpenDCA(symbol,PERIOD_M15,last_time_sel)) //
        {
         ...
         bool opened_sel = //Open_Position(symbol,TREND_SEL,vol,0.0,0.0,comment_sel);
            m_trade.SellLimit(vol,ask+0.5,symbol,0.0,TP_SEL,0,0,comment_sel);
...
//|                                                                  |
//+------------------------------------------------------------------+
void ModifyTp2Entry(string symbol, string TREND, string MASK)
  {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
      if(m_position.SelectByIndex(i))
         if(is_same_symbol(m_position.Comment(), BOT_SHORT_NM) &&
            ...
            is_same_symbol(m_position.Symbol(), symbol) &&
            is_same_symbol(m_position.Comment(),TREND) &&
            is_same_symbol(m_position.TypeDescription(),trend_type))
           {
