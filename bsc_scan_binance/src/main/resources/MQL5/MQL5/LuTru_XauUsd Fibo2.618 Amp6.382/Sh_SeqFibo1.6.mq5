...
double AMP_TP_L1=3.0;
double AMP_HEDGING=3.0;
double AMP_TP_Lx=AMP_TRADE*FIBO;
//-----------------------------------------------------------------------------
double AMP_STOP=300;
...
int OnInit()
  {
   ...
   FindMACDExtremes(symbol,PERIOD_CURRENT);

   if(is_same_symbol(symbol,"XAU"))
     {
      AMP_TRADE=3;
      AMP_TP_L1=AMP_TRADE;
      AMP_HEDGING=AMP_TRADE;
      AMP_TP_Lx=AMP_TRADE*FIBO;
     }
...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   string Trend_Heiken_H1C0=get_trend_by_heiken(symbol,PERIOD_H1,0);
   string Trend_Heiken_M1C0=get_trend_by_heiken(symbol,PERIOD_M1,0);

   string TREND="h4 (Ma:"+getShortName(Trend_Ma10_H4C0)+"  Hei:"+getShortName(Trend_Heiken_H4C0)+")"
                +"  h1:"+getShortName(Trend_Heiken_H1C0)
                +"  m1:"+getShortName(Trend_Heiken_M1C0)+"  |"+format_double_to_string(moved,digits-2)+"|";

   if(is_same_symbol(Trend_Ma10_H4C0+Trend_Heiken_H4C0+Trend_Heiken_H1C0+Trend_Heiken_M1C0,TREND_SEL))
      IS_CYCLE_BUY=false;
   if(is_same_symbol(Trend_Ma10_H4C0+Trend_Heiken_H4C0+Trend_Heiken_H1C0+Trend_Heiken_M1C0,TREND_BUY))
      IS_CYCLE_SEL=false;
//---------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------
   ...
   create_lable_simple("AVG_BUY","-----(AvgB "+format_double_to_string(Avg_Price_Buy,digits-1)+")-----",Avg_Price_Buy,clrBlue,     time+TIME_OF_ONE_D1_CANDLE);
   create_lable_simple("AVG_SEL","-----(AvgS "+format_double_to_string(Avg_Price_Sel,digits-1)+")-----",Avg_Price_Sel,clrFireBrick,time+TIME_OF_ONE_D1_CANDLE);
//---------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------------
   if((profit_sel+profit_buy>risk_1L) && (count_buy+count_sel>=MathMin(EXIT_L,7)))
     {
...
//---------------------------------------------------------------------------------------------------------
   if(IS_CYCLE_BUY && (count_buy==0) && (Trend_Heiken_H4C0==TREND_BUY))
     {
      count_buy=1;
   ...
   if(IS_CYCLE_SEL && (count_sel==0) && (Trend_Heiken_H4C0==TREND_SEL))
     {
      count_sel=1;
...
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      if((global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask) && CanOpenDCA(symbol,PERIOD_H1,last_time_buy))
        {
         count_buy+=1;
      ...
      if(profit_buy>risk_1L)
        {
         if((count_buy>=MathMin(EXIT_L,7)) || (IS_CYCLE_BUY==false))
           {
            if(MathAbs(global_min_entry_buy-cur_price) > GetGlobalVariable(MAX_AMP_SOLVE_BUY))
               SetGlobalVariable(MAX_AMP_SOLVE_BUY, MathAbs(global_min_entry_buy-cur_price));
...
//---------------------------------------------------------------------------------------------------------
   if(count_sel>0)
     {
      if((global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid) && CanOpenDCA(symbol,PERIOD_H1,last_time_sel))
        {
         count_sel+=1;
      ...
      if(profit_sel>risk_1L)
        {
         if((count_sel>=MathMin(EXIT_L,7)) || (IS_CYCLE_SEL==false))
           {
            if(MathAbs(global_max_entry_sel-cur_price) > GetGlobalVariable(MAX_AMP_SOLVE_SEL))
               SetGlobalVariable(MAX_AMP_SOLVE_SEL, MathAbs(global_max_entry_sel-cur_price));
   ...
   if(false)
     {
      if(count_buy>0 && is_same_symbol(total_comment_buy,comment_L001_buy)==false)
        {
         ClosePositionByTrend(symbol,TREND_BUY);
         Alert("STOP_TRADE_L001 "+symbol+"  "+total_comment_buy);
         return;
        }
      ...
      if(count_sel>0 && is_same_symbol(total_comment_sel,comment_L001_sel)==false)
        {
         ClosePositionByTrend(symbol,TREND_SEL);
         Alert("STOP_TRADE_L001 "+symbol+"  "+total_comment_sel);
         return;
        }
     }
//---------------------------------------------------------------------------------------------------------
   ...
   if(ALLOW_DRAW_BUTONS && is_cur_tab)
     {
      ...
      createButton("TREND",TREND,x_ref_btn/2-100,y_ref_btn-35,200,30,clrBlack,is_same_symbol(Trend_Ma10_H4C0,TREND_BUY)?clrLightGreen:clrMistyRose,7);
      ...
      string lblAutoSel=(GetGlobalVariable(BtnAutoSel+symbol)==AUTO_TRADE_ON?"(Global ON)":"(Global Off)")+" Sell ";
      createButton(BtnAutoSel,lblAutoSel,x_ref_btn-120,40,120,30,clrBlack,IS_CYCLE_SEL?clrLightGreen:clrLightGray);

      string lblAutoBuy=(GetGlobalVariable(BtnAutoBuy+symbol)==AUTO_TRADE_ON?"(Global ON)":"(Global Off)")+" Buy ";
      createButton(BtnAutoBuy,lblAutoBuy,x_ref_btn-120,y_ref_btn-50,120,30,clrBlack,IS_CYCLE_BUY?clrLightGreen:clrLightGray);


      string lblProfitBuy="";
      if(count_buy>0)
         lblProfitBuy=" "+DoubleToString(vol_buy,2)+".lot "+DoubleToString(profit_buy,2)+"$";
      createButton(BtnTpPositiveBuy,lblProfitBuy,x_ref_btn-120,y_ref_btn-110,120,20,profit_buy>0?clrBlue:clrRed,clrWhite,7);

      string lblProfitSel="";
      if(count_sel>0)
         lblProfitSel=" "+DoubleToString(vol_sel,2)+".lot "+DoubleToString(profit_sel,2)+"$";
      ...
      createButton(BtnSendLimitBuy,"Limit.5L.Buy."+format_double_to_string(AMP_TP_L1,digits-1),x_ref_btn-120,y_ref_btn-150,120,20,clrBlack,clrLightGray,7);
      createButton(BtnSendLimitSel,"Limit.5L.Sel."+format_double_to_string(AMP_TP_L1,digits-1),x_ref_btn-120,150,120,20,clrBlack,clrLightGray,7);
...
// Hàm tìm nến có Histogram cao nhất khi dương và thấp nhất khi âm
void FindMACDExtremes(string symbol, ENUM_TIMEFRAMES timeframe)
  {
      ...
      if(is_positive_wave && macd_histogram < 0)
        {
         ...
         create_vertical_line("MACD_HIG_" + (string)highest_positive_index, time,clrRed,STYLE_SOLID,1);
         create_trend_line("MACD_HIG_" + (string)highest_positive_index+"_",time-add_time*2,hig, time+add_time*2,hig,clrRed,STYLE_SOLID);
      ...
      if(is_negative_wave && macd_histogram > 0)
        {
         ...
         create_vertical_line("MACD_LOW_" + (string)lowest_negative_index,time, clrBlue,STYLE_SOLID,1);
         create_trend_line("MACD_LOW_" + (string)lowest_negative_index+"_",time-add_time*4,low,time+add_time*4,low,clrBlue,STYLE_SOLID);
...
string GetComments()
  {
   ...
   str_comments+="    Min: "+format_double_to_string(Risk_Min(),2)+"$ Exit>="+(string)EXIT_L+"L";
   ...
   str_comments+="\n";
   str_comments+="    AVG_W1: "+(string)GetGlobalVariable(AVG_HEIGHT_W1+symbol)+" ("+(string)num_candles_w1+")";
   str_comments+="    AVG_D1: "+(string)GetGlobalVariable(AVG_HEIGHT_D1+symbol)+" ("+(string)num_candles_d1+")";
   str_comments+="    AVG_H4: "+(string)GetGlobalVariable(AVG_HEIGHT_H4+symbol)+" ("+(string)num_candles_h4+")";
   str_comments+="    AVG_H1: "+(string)GetGlobalVariable(AVG_HEIGHT_H1+symbol)+" ("+(string)num_candles_h1+")";
