...
double AMP_STOP = AMP_TP_L1*6; //AMP30$ là cháy.
double AMP_TRAILING_STOP = 1.68;
double AMP_MOVE_TP=AMP_TRAILING_STOP*2;
double FIBO = 1.618;
...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   double price_lowest_2d=MathMin(bid-0.25,MathMin(iLow(symbol,PERIOD_D1,0),iLow(symbol,PERIOD_D1,1)));
   double price_higest_2d=MathMax(ask+0.25,MathMax(iHigh(symbol,PERIOD_D1,0),iHigh(symbol,PERIOD_D1,1)));
   ...
   if(IS_MACD_FOLLOWING)
     {
      trend_following = get_trend_by_ma(symbol,PERIOD_D1,10,1);

      if(trend_following==TREND_SEL)
         IS_CYCLE_BUY=false;
      if(trend_following==TREND_BUY)
         IS_CYCLE_SEL=false;
     }
...
//---------------------------------------------------------------------------------------------------------
   if(count_buy>0)
     {
      if((global_min_entry_buy>0) && (global_min_entry_buy-AMP_TRADE>ask)) //IS_CYCLE_BUY &&
        {
         count_buy += 1;
...
//---------------------------------------------------------------------------------------------------------
   if(count_sel>0)
     {
      if((global_max_entry_sel>0) && (global_max_entry_sel+AMP_TRADE<bid)) //IS_CYCLE_SEL &&
        {
         count_sel += 1;
   ...
   if(is_cur_tab)
     {
      ...
      string strLblTrendFollowing = "(Follow " + get_time_frame_name(PERIOD_MACD_FOLLOWING) + ") " + trend_following + " " + (IS_MACD_FOLLOWING?"ON":"OFF");
      ...
      string stoc_main = "(" + format_double_to_string(AMP_TRADE,digits-1) + "/" + format_double_to_string(AMP_STOP,digits-1) + ")";
