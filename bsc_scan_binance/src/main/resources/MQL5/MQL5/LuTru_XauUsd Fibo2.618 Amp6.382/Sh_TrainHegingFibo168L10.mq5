...
void OpenTrade_XauUsd(string symbol)
  {
   ...
   bool IS_FOLLOW_TREND = GetGlobalVariable(BtnTrendFollow+symbol)==AUTO_TRADE_ON || IS_TESTING;
   ...
   if(IS_FOLLOW_TREND)
     {
      string trend_ma10_w1 = get_trend_by_ma(symbol,PERIOD_W1,10,1);
   ...
   if(count_buy>0)
     {
      ...
      if(profit_buy>risk_min)
        {
         if((count_buy>=START_EXIT_L) || (count_buy>=7) || (IS_CYCLE_BUY==false))
           {
   ...
   if(count_sel>0)
     {
      ...
      if(profit_sel>risk_min)
        {
         if((count_sel>=START_EXIT_L) || (count_sel>=7) || (IS_CYCLE_SEL==false))
           {
