...
public class BinanceServiceImpl implements BinanceService {
    ...
    public List<CandidateTokenCssResponse> getList(Boolean isOrderByBynaceVolume) {
        try {
            ...
            String sql = " select                                                                                 \n"
                    ...
                    + "             ROUND((select MIN(COALESCE(w.min_price, 1000000)) from binance_volumn_week w where w.gecko_id = can.gecko_id and w.symbol = can.symbol and yyyymmdd between TO_CHAR(NOW() - interval '14 days', 'yyyyMMdd') and TO_CHAR(NOW(), 'yyyyMMdd')), 5) as avg21d, \n" //min14d
            ...
            for (CandidateTokenResponse dto : results) {
...

                {
                    ...
                    //avg_history += ", min14d: " + Utils.removeLastZero(dto.getAvg21d().toString()) + "("
                    //        + Utils.toPercent(dto.getAvg21d(), price_now) + "%)";

                    avg_history += ", max28d: " + Utils.removeLastZero(dto.getAvg14d().toString()) + "("
    ...
    public String loadPremarket() {
        String sp500 = getPreMarket("https://markets.businessinsider.com/futures/s&p-500-futures");
        String dow_jones = getPreMarket("https://markets.businessinsider.com/futures/dow-futures");
        String nasdaq = getPreMarket("https://markets.businessinsider.com/futures/nasdaq-100-futures");
