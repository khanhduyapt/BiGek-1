                ...
                if (!Objects.equals(null, dto.getPrice_can_buy()) && !Objects.equals(null, dto.getPrice_can_sell())
                    ...
                    if (Objects.equals("BTC", dto.getSymbol().toUpperCase())) {
                        ...
                        if (!Objects.equals(curr_time_of_btc, pre_time_of_btc)) {
                            ...
                            BigDecimal btc_range_l_h = ((highest_price_today.subtract(lowest_price_today))
                                    .divide(price_now, 3, RoundingMode.CEILING));

                            BigDecimal price_sell = dto.getPrice_can_sell();
                            BigDecimal price_buy = dto.getPrice_can_buy();
                            BigDecimal btc_range_b_s = ((price_sell.subtract(price_buy)).divide(price_buy, 3,
                                    RoundingMode.CEILING));

                            if ((btc_range_b_s.compareTo(BigDecimal.valueOf(0.02)) >= 0)
                                    || (btc_range_l_h.compareTo(BigDecimal.valueOf(0.015)) >= 0)) {

                                if (Utils.isGoodPrice(price_now, price_buy, price_sell)
                                        || Utils.isGoodPrice(price_now, lowest_price_today, highest_price_today)) {
            ...
            if ((percent_loss.compareTo(BigDecimal.valueOf(-0.85)) > 0)
                ...
                if (binanceFuturesRepository.existsById(dto.getGecko_id())) {
                    ...
                    if (!Objects.equals(null, today)
                            && Utils.getBigDecimal(today.getMin_price()).compareTo(BigDecimal.valueOf(-1)) > 0) {
